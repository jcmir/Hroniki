...
pub async fn seed_demo_data(state: State<'_, AppState>) -> Result<(), String> {
    ...
    // 2. Insert Categories
    sqlx::query("INSERT INTO categories (id, name, icon, theme) VALUES ('cat-garden', 'Сад', '🌿', 'emerald')").execute(&mut *tx).await.map_err(|e| e.to_string())?;
    sqlx::query("INSERT INTO categories (id, name, icon, theme) VALUES ('cat-home', 'Дом', '🏡', 'blue')").execute(&mut *tx).await.map_err(|e| e.to_string())?;
    sqlx::query("INSERT INTO categories (id, name, icon, theme) VALUES ('cat-auto', 'Имущество', '🚗', 'indigo')").execute(&mut *tx).await.map_err(|e| e.to_string())?;
    ...
    // 4. Insert Entries
    // Garden
    sqlx::query("INSERT INTO entries (id, object_id, occurred_at, title, description) VALUES ('ent-g1', 'obj-apple', ?, 'Посадка саженца Антоновки', 'Посадили молодой саженец с детьми. Обильно полили, добавили торф и укрепили колышком.')")
        .bind((Utc::now() - chrono::Duration::days(365)).to_rfc3339())
        .execute(&mut *tx)
        .await
        .map_err(|e| e.to_string())?;

    sqlx::query("INSERT INTO entries (id, object_id, occurred_at, title, description) VALUES ('ent-g2', 'obj-apple', ?, 'Весенняя побелка ствола', 'Побелили ствол для защиты от грызунов и весенних ожогов. Добавили медный купорос.')")
        .bind((Utc::now() - chrono::Duration::days(60)).to_rfc3339())
        .execute(&mut *tx)
        .await
        .map_err(|e| e.to_string())?;

    sqlx::query("INSERT INTO entries (id, object_id, occurred_at, title, description) VALUES ('ent-g3', 'obj-apple', ?, 'Первый урожай яблок!', 'Собрали первые 5 спелых яблок! Очень сочные, с легкой кислинкой. Растет отлично.')")
    ...
    // Home
    sqlx::query("INSERT INTO entries (id, object_id, occurred_at, title, description) VALUES ('ent-h1', 'obj-thermostat', ?, 'Установка термостата', 'Смонтировали беспроводной умный термостат. Настроили автоматическое расписание: +21°C днем и +18°C ночью.')")
    ...
    // Auto
    sqlx::query("INSERT INTO entries (id, object_id, occurred_at, title, description) VALUES ('ent-a1', 'obj-car', ?, 'Сезонное ТО и замена масла', 'Заменили моторное масло (5W-30), масляный, воздушный и салонный фильтры. Износ тормозных колодок 35%.')")
    ...
    // 5. Insert Photos
    sqlx::query("INSERT INTO photos (id, entry_id, path, thumbnail_path, created_at) VALUES ('photo-demo', 'ent-g3', 'demo_apple.jpg', 'demo_apple.jpg', ?)")
