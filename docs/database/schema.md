...
### 1.11 Таблица `audit_logs`
Журналирование действий пользователей и событий безопасности.
- `id` (TEXT PRIMARY KEY, NOT NULL): Уникальный UUID записи.
- `user_id` (TEXT): Идентификатор пользователя, инициировавшего действие (может быть NULL для пред-авторизационных событий). Связь с `users.id` (внешний ключ `ON DELETE CASCADE`).
- `event_type` (TEXT NOT NULL): Имя доменного события.
- `details` (TEXT): Сериализованные детали события в формате JSON.
- `created_at` (TEXT NOT NULL): Дата логирования.
...
- **`0007_audit_log.sql`**: Создает таблицу логов аудита (`audit_logs`) для записи доменных событий безопасности.
